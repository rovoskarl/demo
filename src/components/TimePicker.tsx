...
export function TimePicker({
  ...
  if (hoursInterval > MAX_HOURS || minutesInterval > MAX_MINUTES || secondsInterval > MAX_SECONDS) {
  ...
  const getHoursItems = () => {
    ...
    if (emptyLabel != null) {
      items.push(<Picker.Item testID="hoursItem" key="nullHoursItem" value="" label={emptyLabel} color={textColor} />);
    }

    for (let i = MIN_HOURS; i <= MAX_HOURS; i += hoursInterval) {
      items.push(<Picker.Item testID="hoursItem" key={i} value={i} label={getLabel(i, hoursUnit)} color={textColor} />);
    }
    return items;
  };
  ...
  const getMinutesItems = () => {
    ...
    if (emptyLabel != null) {
      items.push(
        <Picker.Item testID="minutesItem" key="nullMinutesItem" value="" label={emptyLabel} color={textColor} />,
      );
    }

    for (let i = 0; i <= MAX_MINUTES; i += minutesInterval) {
      items.push(
        <Picker.Item testID="minutesItem" key={i} value={i} label={getLabel(i, minutesUnit)} color={textColor} />,
      );
    }
    return items;
  };
  ...
  const getSecondsItems = () => {
    ...
    if (emptyLabel != null) {
      items.push(
        <Picker.Item testID="secondsItem" key="nullSecondsItem" value="" label={emptyLabel} color={textColor} />,
      );
    }

    for (let i = 0; i <= MAX_SECONDS; i += secondsInterval) {
      items.push(
        <Picker.Item testID="secondsItem" key={i} value={i} label={getLabel(i, secondsUnit)} color={textColor} />,
      );
    }
    return items;
  };
  ...
  return (
    <View style={styles.container}>
      ...
      {isAmpm && (
        ...
        >
          <Picker.Item testID="amItem" value="am" label={ampmLocalization.am} color={textColor} />
          <Picker.Item testID="pmItem" value="pm" label={ampmLocalization.pm} color={textColor} />
        </Picker>
      )}
    </View>
  );
